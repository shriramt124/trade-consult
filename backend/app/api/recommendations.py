...
def _subscribed_plan_ids(db: Session, user: User) -> list[int]:
    if user.role in (UserRole.analyst, UserRole.admin):
        rows = db.scalars(select(Subscription.plan_id).distinct()).all()
        return list(rows)
    rows = db.scalars(
...
def live_calls(
    ...
    """Open calls for the plans the current user is actively subscribed to."""
    plan_ids = _subscribed_plan_ids(db, user)
    if not plan_ids:
        raise HTTPException(
            status_code=403,
...
def my_history(
    ...
    return db.scalars(
        select(Recommendation)
        .where(Recommendation.plan_id.in_(plan_ids))
        .order_by(Recommendation.created_at.desc())
        .limit(100)
    ).all()
