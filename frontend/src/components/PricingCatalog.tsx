...
function Card({ plan, periodLabel }: { plan: Plan; periodLabel: string }) {
  return (
    <div className="flex flex-col justify-between rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
      <div>
        <div className="flex items-baseline gap-1.5">
          <span className="text-2xl font-bold text-slate-950">{formatINR(plan.price_paise)}</span>
      ...
      <a
        ...
        className="mt-5 w-full rounded-full bg-slate-950 py-2.5 text-center text-sm font-medium text-white transition hover:bg-red-600"
      >
        Choose Pricing Plan
      </a>
    </div>
    ...
    <div className="mt-4 w-full">
      ...
      {flagship && (
        <section className="mt-14 w-full text-left">
          ...
          <h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Insiight 360</h2>
          <div className="mt-6 max-w-xl rounded-2xl border border-red-200 bg-white p-5 sm:p-6">
            <div className="flex items-baseline gap-1.5">
              <span className="text-3xl font-bold text-slate-950">{formatINR(flagship.price_paise)}</span>
              <span className="text-xs text-slate-500">+18% GST</span>
            </div>
